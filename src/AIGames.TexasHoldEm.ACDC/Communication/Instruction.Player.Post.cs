...
{
	public struct PostInstruction : IInstruction
	{
		public PostInstruction(PlayerName name, int value)
		{
			m_Name = name;
			m_Value = value; 
		}
		...
		public override string ToString() { return String.Format("{0} post {1}", Name, Value); }
		...
		internal static IInstruction Parse(PlayerName name, string[] splited)
		{
			...
			{
				return new PostInstruction(name, value);
			}
			return null;
		}
	}
