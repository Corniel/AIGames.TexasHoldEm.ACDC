...
{
	public struct WinsInstruction : IInstruction
	{
		public WinsInstruction(PlayerName name, int value)
		{
			m_Name = name;
			m_Value = value; 
		}
		...
		public override string ToString() { return String.Format("{0} wins {1}", Name, Value); }
		...
		internal static IInstruction Parse(PlayerName name, string[] splited)
		{
			...
			{
				return new WinsInstruction(name, value);
			}
			return null;
		}
	}
